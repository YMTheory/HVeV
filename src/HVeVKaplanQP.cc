...
AbsorbPhonon(G4double energy, std::vector<G4double>& reflectedEnergies) const {
  ...
  if (!ParamsReady()) {
    G4Exception("HVeVKaplanQP::AbsorbPhonon()", "G4CMP001",
                RunMustBeAborted, "Thin film parameters not properly set.");
  }

  if (verboseLevel)
    G4cout << "HVeVKaplanQP::AbsorbPhonon " << energy << G4endl;

  if (reflectedEnergies.size() > 0) {
    G4Exception("HVeVKaplanQP::AbsorbPhonon", "G4CMP007", JustWarning,
  ...
  //if (verboseLevel && !output.is_open()) {
    ...
    if (!output.good()) {
      G4Exception("HVeVKaplanQP", "G4CMP008",
  ...
  G4cout << "Total " << energy/eV << "eV absorbed and split into " << nQPpairs << " QP pairs." << G4endl;
  ...
  G4cout << "Edep in this process: " << EDep/eV << " eV, and reflected energy is " << ERefl/eV << " eV." << G4endl;
  return EDep;
}
...
ReportAbsorption(G4double energy, G4double EDep,
  ...
  if (delta < 0.) {		// Actual energy excess
    G4cerr << "WARNING HVeVKaplanQP has excess " << delta/eV << " eV"
...
G4double HVeVKaplanQP::CalcEscapeProbability(G4double energy,
					      G4double thicknessFrac) const {
  if (verboseLevel>1) {
    G4cout << "HVeVKaplanQP::CalcEscapeProbability E " << energy
...
G4double
HVeVKaplanQP::CalcQPEnergies(std::vector<G4double>& phonEnergies,
			      std::vector<G4double>& qpEnergies) const {
  if (verboseLevel>1) {
    G4cout << "HVeVKaplanQP::CalcQPEnergies QPcut " << lowQPLimit*gapEnergy
	   << G4endl;
  }
...
G4double
HVeVKaplanQP::CalcPhononEnergies(std::vector<G4double>& phonEnergies,
				  std::vector<G4double>& qpEnergies) const {
  if (verboseLevel>1) {
    G4cout << "HVeVKaplanQP::CalcPhononEnergies 2*gap " << 2.*gapEnergy
...
CalcReflectedPhononEnergies(std::vector<G4double>& phonEnergies,
  ...
  if (verboseLevel>1)
    G4cout << "HVeVKaplanQP::CalcReflectedPhononEnergies " << G4endl;
...
G4bool HVeVKaplanQP::DoDirectAbsorption(G4double energy) const {
  if (verboseLevel>1) {
    G4cout << "HVeVKaplanQP::DoDirectAbsorption E " << energy
...
G4double
HVeVKaplanQP::CalcDirectAbsorption(G4double energy,
...
G4double
HVeVKaplanQP::CalcQPAbsorption(G4double qpE,
